...
def dnsIterate(targetWeb, index, dtype, root_servers):#pir.org
	...
	#print response.answer[0][0]

	key_rrset = response.answer[0]
	dnskey_rrsig = response.answer[1][0]
	key_dict = {dns.name.from_text(domain): key_rrset}
	try:
		dns.dnssec.validate(key_rrset, dnskey_rrsig, key_dict)
		print "pass"
	except:
		print "xxxx"
